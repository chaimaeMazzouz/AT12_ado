...
namespace AT12
{
    public partial class mouvementDepot : Form
    {
        banqueEntities Bq;
        ...
        private void mouvementDepot_Load(object sender, EventArgs e)
        {
        ...
        private void btbDepot_Click(object sender, EventArgs e)
        {
            if (comboNuComp.Text != "" && comboNuComp.Text != "--Faites votre choix--" && textMontant.Text != "")
            {
                Bq.ps_Depot(int.Parse(comboNuComp.SelectedValue.ToString()), decimal.Parse(textMontant.Text));
